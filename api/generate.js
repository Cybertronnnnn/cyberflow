const { sb, auth, groq, cors, parseBody } = require('./_shared');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    res.json({ content, generation_id: saved?.id, quota: { used: newCount, limit: p.plan === 'pro' ? null : FREE_LIMIT, remaining: p.plan === 'pro' ? null : Math.max(0, FREE_LIMIT - newCount) } });
  } catch(e) {
